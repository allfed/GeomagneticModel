...
def fitPower(xdata, ydata):
    ...
    residuals = ydata - powerlaw(xdata, exponent)
    ss_res = np.sum(residuals**2)
    ss_tot = np.sum((ydata - np.mean(ydata)) ** 2)
    r_squared = 1 - (ss_res / ss_tot)
    print("powerfit rquared: ", r_squared)
...
def binnormaldist(dist, counts, sigmas):
    ...
    return [
        np.sqrt(np.multiply(np.array(binvalslow), np.array(binvalshigh))),
        countsnormalized,
    ]
...
def binlognormaldist(dist, counts, sigmas):
    ...
    for i in range(0, nbins):
        m = 1 / np.exp(nbins)
        low = (np.exp(i) / np.exp(nbins)) ** (
            np.log(minimum / maximum) / np.log(m)
        ) * maximum
        high = (np.exp(i + 1) / np.exp(nbins)) ** (
            np.log(minimum / maximum) / np.log(m)
        ) * maximum
        mask = np.logical_and(dist > low, dist < high)
    ...
    countsnormalized = np.array(bins) / np.sum(bins)
    return [
        np.divide(np.array(binvalslow) + np.array(binvalshigh), 2),
        countsnormalized,
    ]
...
def combinecounts(E, allcountsatE):
    ...
    combined = (
        pd.DataFrame({"counts": sortedcountsbyE, "negE": negE})
        .groupby(by=["negE"])
        .sum()
    )
...
def getEfieldCounts(peaks):
    ...
    combined = (
        pd.DataFrame({"negE": negE, "counts": np.ones(len(negE))})
        .groupby(by=["negE"])
        .sum()
    )
    countsfinal = combined["counts"].values
