...
class Network:
	def __init__(self):
		...
		lineConductance=1/np.array([.037,.072/3,.037/3,.013/3,.011/3])#Ohms/km/phase
		self.linekV=linekV
	...
	def importNetwork(self,continent,country):
		...
		print('')
		# self.analyzeNetwork()
		Plotter.plotNetwork(self.voltages,self.lines)
	...
	def importConnections(self,nodesDict):
		...
		for c in self.conns:
			...
			#get the substation name for each winding
			if(v in self.lineResistance.keys()):
				resistancePerKilometer=self.lineResistance[v]
			else:
				self.lineResistance[v]=1/np.interp(v,self.linekV,self.lineConductance)
				resistancePerKilometer=self.lineResistance[v]

			r=resistancePerKilometer*(c.length/100) #Ohms (net resistance of the power line, length converted to kilometers)
			index = index + 1
			...
			#if we haven't connected winding1 to the substation ground yet
			if(float(index1)==2633.0):
				print(index1 in isConnectedToGround.keys())
				print('^index1 is 2633')
				print('')
				print('')
				print('')
				print('')
				print('')
				print('')
			if(float(index2)==2633.0):
				print(index2 in isConnectedToGround.keys())
				print('^index2 is 2633')
				print('')
				print('')
				print('')
				print('')
			...
			if(not (index1 in isConnectedToGround.keys())):
				...
				outputConnections.append([index,index1,sub1index,'0','0',self.transformertogroundR,'0'])
			...
			if(not (index2 in isConnectedToGround.keys())):
				...
				# __0__ ____1____ ___2___ _3_ _4_ ________5_______ _6_
				# index nodefrom  nodeto   0   0  wind resistance   0
				outputConnections.append([index,index2,sub2index,'0','0',self.transformertogroundR,'0'])
