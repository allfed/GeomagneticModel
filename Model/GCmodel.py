...
class GCmodel:
    def __init__(self):
        ...
        if not self.downloaded:
            ...
            print(
                "Warning: Missing global conductivity model. Expected location in filesystem: "
                + currentdir
                + "/"
                + Params.globalcondloc
            )
            print(
                "It should be the matlab version, with filename renamed from MODEL-VER-[version].mat to 'GCmodel.mat'. Dowload it from https://globalconductivity.ocean.ru/"
            )
    ...
    def importGCmodel(self):
        ...
        with h5py.File(Params.globalcondloc, "r") as f:
            ...
            self.deltas = np.diff(
                np.transpose(self.zgrid)
            )  # the difference between heights of layers
            self.depths = np.insert(
                self.deltas[0], 0, 100.0, axis=0
            )  # the first layer is 100m thick
    ...
    def getImpedanceMap(self):
        latres = Params.latituderes
        longres = Params.longituderes
        latInterval = int(np.floor(latres / 0.25))
        longInterval = int(np.floor(longres / 0.25))
        maxcond = -1e19
        # maximp = -1e19
        mincond = 1e19
        # minimp = 1e19
        ...
        with h5py.File(Params.globalcondloc, "r") as f:
            ...
            condArr = np.zeros(
                (
                    int(np.floor(lenlong / longInterval)),
                    int(np.floor(lenlat / latInterval)),
                )
            )
            ...
            for longkey in range(0, lenlong - 1, longInterval):
                for latkey in range(0, lenlat, latInterval):
                    ...
                    compleximp = self.getImpedance(c)
                ...
                row = 0
                col = col + 1
            np.save(Params.apparentcondloc, condArr)
            # np.save('imp',impArr)

            condForShow = np.flipud(
                (np.log(condArr) - np.log(mincond))
                / (np.log(maxcond) - np.log(mincond))
            )
