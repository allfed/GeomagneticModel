...
class GCmodel:
	...
	def importGCmodel(self):
		...
		with h5py.File(Params.globalcondloc, 'r') as f:
			...
			self.depths = np.insert(self.deltas[0], 0, 100., axis=0) #the first layer is 100m thick
	...
	def getImpedanceMap(self):
		...
		with h5py.File(Params.globalcondloc, 'r') as f:
			...
			condArr = np.zeros((int(np.floor(lenlong/longInterval)), int(np.floor(lenlat/latInterval))))
