...
class TFsite:
    ...
    def importsite(self):
        ...
        self.wInEDI = (
            []
        )  # the EDI file has its own list of frequencies, which we use as approximations to ours
        self.rows = (
            []
        )  # the EDI file frequencies are listed on specific rows and columns in the data.
        self.rowlen = 0
        self.cols = (
            []
        )  # the EDI file frequencies are listed on specific rows and columns in the data.
        ...
        if self.TFsitefn:
            if self.tfformat == "xml":
                self.importXML()
                self.wProvided = [
                    x * 2.0 * np.pi for x in self.f
                ]  # rad s^-1 (angular frequency), list of frequencies provided
        ...
        try:
            while True:
                ...
                if line.lstrip().find("LAT=") == 0:
                    ...
                    if numarray[0] < 0:
                        self.lat = (
                            numarray[0] - numarray[1] / (60) - (numarray[2]) / (60 * 60)
                        )
                    else:
                        self.lat = (
                            numarray[0] + numarray[1] / (60) + (numarray[2]) / (60 * 60)
        ...
        try:
            while True:
                ...
                if line.find("****FREQUENCIES****") > -1:
                    line = (
                        file.readline()
                    )  # skip '>FREQ //[number of frequencies]' line
    ...
    def calcApparentr(self):
        ...
        for i in range(0, len(w)):
            # https://mathworld.wolfram.com/FrobeniusNorm.html
            # The frobenius norm involves absolute value squared of matrix elements, adding, then taking the square root.
            self.rhoXX = self.rhoXX + [
                ZXX[i] * np.conj(ZXX[i]) / (2 * np.pi * u0 * self.f[i])
            ]
            self.rhoXY = self.rhoXY + [
                ZXY[i] * np.conj(ZXY[i]) / (2 * np.pi * u0 * self.f[i])
            ]
            self.rhoYX = self.rhoYX + [
                ZYX[i] * np.conj(ZYX[i]) / (2 * np.pi * u0 * self.f[i])
            ]
            self.rhoYY = self.rhoYY + [
                ZYY[i] * np.conj(ZYY[i]) / (2 * np.pi * u0 * self.f[i])
            ]
